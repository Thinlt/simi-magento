require(['jquery', 'jquery/ui','mage/mage','mage/validation'], function($){
	...
	for (i = 0; i < x.length; i++) {
		...
		a.addEventListener("click", function(e) {
			...
			if(jQuery(".reset-type").val()!="Mobile") {
				jQuery('#resetbyemail').val(document.getElementById("select-selected").innerText);
				...
				jQuery(".forgotsendotp").val(jQuery(".forgotsendotp").val());
	...
	if(jQuery("#backendlogintype").val()==0)
	{
		...
		jQuery('.login-container:first').hide();
	}
	else
	{
		jQuery('#login-type li #loginwithotp').removeClass('inactive');
		jQuery('.login-container:last').hide();
	}
...
function doRegister()
{
	...
	jQuery(".verifyotp").css('display','none');
	jQuery("#otp-cont").css('display','none');
	jQuery(".sendotp").val(jQuery(".sendotp.mobbtn").val());
...
function doLogin()
{
	...
	jQuery(".error1").css('display','none');
	jQuery(".ajax-login-form").css('display','block');
	jQuery(".loginotpmobbtn").val(jQuery(".sendotp.mobbtn").val());
