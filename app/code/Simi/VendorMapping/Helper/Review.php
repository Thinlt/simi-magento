...
class Review extends \Simi\Simiconnector\Helper\Data
{
    ...
    protected function _getRestData($avg, $ratings, $getForm = true){
        return [
            'rate'             => $avg,
            'number'           => $ratings[5],
     ...
     */
    public function getVendorReviews($vendorId, $getForm = true) {
        $ratings      = $this->getVendorRatingStar($vendorId);
    ...
    /**
     * Get rating star by Vendor ID for all products
     */
    public function getVendorRatingStar($vendorId){
        $storeId = $this->storeManager->getStore()->getId();
        $reviews = $this->simiObjectManager->get('Simi\VendorMapping\Model\Review')
                ->getResourceCollection()
                ->addStoreFilter($storeId)
    ...
    public function getRatingStar($reviews)
    {
        /**
