...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This newcollections no longer exists.'));
        ...
        $this->coreRegistry->register('newcollections', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Newcollections') : __('New Newcollections'),
            $id ? __('Edit Newcollections') : __('New Newcollections')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Newcollections'));
        $resultPage->getConfig()->getTitle()
                ->prepend($model->getId() ? $model->getId() : __('New Newcollections'));
        return $resultPage;
    }
}
