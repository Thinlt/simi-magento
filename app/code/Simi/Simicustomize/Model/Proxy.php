 ...
 * Class Giftcard
 *
 * @package Simi\Simicustomize\Model\Proxy
 */
class Proxy
{
    /*
    ...
    */
    ...
    public $whitelistPatterns = array(
    ...
    public $landingExampleURL = "https://example.net";
    /****************************** END CONFIGURATION ******************************/
    ...
    public function __construct(){
        ...
        $this->PROXY_PREFIX = "http" . (isset($_SERVER["HTTPS"]) ? "s" : "") . "://" . $prefixHost . $prefixPort . $_SERVER["SCRIPT_NAME"] . "?";
        // define("PROXY_PREFIX", );
    }
    ...
    public function makeRequest($url) {
        ...
        $removedHeaders = $this->removeKeys($browserRequestHeaders, array(
    ...
    public function proxifyCSS($css, $baseURL) {
        ...
        return preg_replace_callback(
            ...
            function($matches) use ($baseURL) {
                ...
                return "url(" . $this->PROXY_PREFIX . $this->rel2abs($url, $baseURL) . ")";
            },
            $normalizedCSS
        );
    }
    ...
    public function proxifySrcset($srcset, $baseURL) {
        ...
        $proxifiedSources = array_map(function($source) use ($baseURL) {
            ...
            $components[0] = $this->PROXY_PREFIX . $this->rel2abs(ltrim($components[0], "/"), $baseURL); //First component of the split source string should be an image URL; proxify it
    ...
    public function query($url = ''){
        ...
        if ($url){
            $this->startURL = $url;
        } elseif (isset($_POST["miniProxyFormAction"])) {
        ...
        if (empty($url)) {
            if (empty($this->startURL)) {
                die("<html><head><title>miniProxy</title></head><body><h1>Welcome to miniProxy!</h1>miniProxy can be directly invoked like this: <a href=\"" . $this->PROXY_PREFIX . $this->landingExampleURL . "\">" . $this->PROXY_PREFIX . $this->landingExampleURL . "</a><br /><br />Or, you can simply enter a URL below:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . $this->PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . $this->PROXY_PREFIX . $this->landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Proxy It!\" /></form></body></html>");
            } else {
                $url = $this->startURL;
            }
        ...
        $urlIsValid = count($this->whitelistPatterns) === 0;
        ...
        if ($responseURL !== $url) {
        header("Location: " . $this->PROXY_PREFIX . $responseURL, true);
            exit(0);
        }
        ...
        if (stripos($contentType, "text/html") !== false) {
            ...
            foreach($xpath->query("//form") as $form) {
                ...
                $form->setAttribute("action", rtrim($this->PROXY_PREFIX, "?"));
            ...
            foreach ($xpath->query("//meta[@http-equiv]") as $element) {
                if (strcasecmp($element->getAttribute("http-equiv"), "refresh") === 0) {
                    ...
                    if (!empty($content)) {
                        ...
                        $element->setAttribute("content", $splitContent[0] . "=" . $this->PROXY_PREFIX . $this->rel2abs($splitContent[1], $url));
                        }
                    }
                }
            }
            ...
            foreach($proxifyAttributes as $attrName) {
                foreach($xpath->query("//*[@" . $attrName . "]") as $element) {
                    ...
                    $attrContent = $this->PROXY_PREFIX . $attrContent;
                    ...
                    '(function() {
                        ...
                        window.XMLHttpRequest.prototype.open = function() {
                            if (arguments[1] !== null && arguments[1] !== undefined) {
                                var url = arguments[1];
                                url = rel2abs("' . $url . '", url);
                                if (url.indexOf("' . $this->PROXY_PREFIX . '") == -1) {
                                url = "' . $this->PROXY_PREFIX . '" + url;
                                }
                                arguments[1] = url;
                            }
            ...
            return "<!-- Proxified page constructed by miniProxy -->\n" . $doc->saveHTML();
        } else if (stripos($contentType, "text/css") !== false) { //This is CSS, so proxify url() references.
            return $this->proxifyCSS($responseBody, $url);
