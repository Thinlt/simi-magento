...
class Urldicts extends \Simi\Simiconnector\Model\Api\Apiabstract
{
    public $params;
    ...
    public function setBuilderQuery(){
        ...
        if (isset($data['resourceid']) && $data['resourceid']) {
            ...
            if (!$this->builderQuery || !$this->builderQuery->getEntityType() || $this->builderQuery->getEntityType() =='cms-page') {
                ...
                /*for cms*/
	            $simi_cms = $this->simiObjectManager
		            ->get('Simi\Simiconnector\Model\Cms')
		            ->getCollection()
		            ->addFieldToFilter('cms_url', $requestPath)->getFirstItem();
	            ...
	            if(strpos( $requestPath , '/' ) !== false){
		            ...
		            if (count($path_rq) >= 2 && $path_rq[0] == 'blog'){
			            // aheadworks url_type 1 : site.com/blog/article
	            ...
	            if ( $simi_cms->getId() ) {
		            $this->builderQuery = $simi_cms;
	            } elseif ($aw_blog && $aw_blog->getId()){
		            $this->builderQuery = $aw_blog;
	            } else {
    ...
    public function show() {
        ...
        if(isset($result['urldict']['product_id']) && $result['urldict']['product_id']) {
            ...
            $data['params']['image_width'] = isset($data['params']['image_width'])?
                $data['params']['image_width']:464;
            $data['params']['image_height'] = isset($data['params']['image_height'])?
                $data['params']['image_height']:579;
