...
window.Variables = {
    ...
    openPdfVariableChooser: function(variables) {
        if (this.variablesContent == null && variables) {
            this.variablesContent = '<ul class="insert-variable vespdf-variables">';
            variables.each(function (variableGroup) {
                //console.log(variableGroup);
                var groupName = variableGroup.label;
                groupName = groupName.toLowerCase().replace(" ", "_");

                this.variablesContent += '<div class="variable--group ' + groupName + '">';
                if (variableGroup.label && variableGroup.value) {
                    this.variablesContent += '<li class="group--title"><b>' + variableGroup.label + '</b></li>';
                    (variableGroup.value).each(function (variable) {
                        if (variable.value && variable.label) {
                            this.variablesContent += '<li class="group--item">' +
                                this.prepareVariableRow(variable.value, variable.label) + '</li>';
                        }
                    }.bind(this));
                }
...
window.MagentovariablePlugin = {
    ...
    openChooser: function(variables) {
        ...
        Variables.openPdfVariableChooser(variables);
    },
    insertVariable : function (value) {
        if (this.textareaId) {
