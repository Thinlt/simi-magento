     ...
     renderBreadcrumb = data => {
         if(data.length > 0){
             ...
             const breadcrumb = data.map((item,key) => {
                 ...
                 let arrow = size === key+1 ? null : <span className="breadcrumb-arrow" style={{margin :'0 5px'}}> > </span>
                 return (
                     <React.Fragment key={key}>
                         <span className="breadcrumb-item" onClick={()=>action()}>
                             {item.name}
                         </span>
                         {arrow}
                     </React.Fragment>
     ...
     renderView = () => {
         ...
         return (
             <div className="container">
                 <div className="harlow-breadcrumb">
