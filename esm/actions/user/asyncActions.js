...
export const signIn = credentials => async function thunk(...args) {
  ...
  try {
    ...
    setToken(response);
    await dispatch(actions.signIn.receive(response)); // Now that we're signed in, get this user's details.

    dispatch(getUserDetails()); // Now that we're signed in, forget the old (guest) cart
