...
export const createCart = () => async function thunk(dispatch, getState) {
  const {
    cart,
    user
  ...
  dispatch(checkoutActions.reset()); // Request a new cart.
  ...
  try {
    ...
    saveCartId(cartId); // There is currently an issue in Magento 2
...
export const addItemToCart = (payload = {}) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      if (!cartId) {
        const missingCartIdError = new Error('Missing required information: cartId');
      ...
      dispatch(actions.addItem.receive(error)); // check if the cart has expired
      ...
      if (noCartId || response && response.status === 404) {
        ...
        await dispatch(createCart()); // then retry this operation
...
export const updateItemInCart = (payload = {}, targetItemId) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      dispatch(actions.updateItem.receive(error)); // check if the cart has expired
...
export const removeItemFromCart = payload => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      dispatch(actions.removeItem.receive(error)); // check if the cart has expired
      ...
      if (noCartId || response && response.status === 404) {
        ...
        // complete before dispatching the error--you don't want an
        // upstream action to try and reuse the known-bad ID.
        await clearCartId(); // then create a new one
...
export const getCartDetails = (payload = {}) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    const {
      isSignedIn
    } = user; // if there isn't a cart, create one
    ...
    try {
      ...
      dispatch(actions.getDetails.receive(error)); // check if the cart has expired
