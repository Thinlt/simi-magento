...
export default function getUrlKey(url = window.location) {
  ...
  const basename = url.pathname.slice(url.pathname.lastIndexOf('/') + 1); // TODO: this may be configurable, but Magento SEO urls appear to always
  // append `.html`, which is not part of the URL key. So strip it.

  return basename.replace(/\.html$/, '');
}
//# sourceMappingURL=getUrlKey.js.map